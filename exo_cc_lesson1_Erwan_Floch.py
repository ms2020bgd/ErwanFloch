...
def pig_latin(text):
    ...
    pig_latin = []
    for mot in mots:
        pig_latin.append(mot[1:] + mot[0].lower() + 'ay')
...
# Given the above data, write a function which returns a list of dict,
...
def extract_data(data):
    ...
    for ville in data.keys():
        dico['name'] = str(ville)
        dico['country'] = weather_data[ville]['metadata']['country']
        dico['date'] = weather_data[ville]['weather_list']['dt_txt']
        dico['temp'] = weather_data[ville]['weather_list']['main']['temp'] - 273.15
    return dico
